...
class MaterialSymbolsMigrationGuardTest {
    ...
    fun `source code does not import compose material icons`() {
            ...
            .flatMap { file ->
                file.readLines().mapIndexedNotNull { index, line ->
                    when {
                        ...
                        Regex(
                            """\bIcons\.(Rounded|Filled|AutoMirrored|Outlined|Sharp|TwoTone)\b""",
        ...
        assertTrue(
            "Use MedLogIcons backed by Material Symbols XML:\n${offenders.joinToString("\n")}",
            offenders.isEmpty(),
        )
    }
    ...
    fun `material symbols are rendered through explicit MedLogIcon API`() {
            ...
            File(projectRoot, "app/src/main/java").walkTopDown()
                ...
                .forEach { file ->
                    file.readLines().forEachIndexed { index, line ->
                        ...
                        if (line.contains("imageVector = MedLogIcons") ||
                            line.contains("imageVector = formIcon") ||
                            line.contains("imageVector = health")
                        ) {
                            add(
                                "${file.relativeTo(
                                    projectRoot,
                                ).path}:${index + 1}: use MedLogIcon(icon = ...) for drawable symbols",
                            )
                        }
                    }
                }
        ...
        assertTrue(
            "Use explicit MedLogIcon API for Material Symbols XML:\n${offenders.joinToString("\n")}",
            offenders.isEmpty(),
        )
    }
    ...
    fun `material symbols source is reproducible`() {
        ...
        assertTrue(
            "Script should source from Google Fonts Material Symbols Rounded endpoints.",
            scriptText.contains("materialsymbolsrounded"),
        )
        assertTrue(
            "Script should explain why the Google Fonts family zip is not used for Android XML.",
            scriptText.contains("download?family=Material+Symbols"),
        )
        assertTrue(
            "MedLogIcons should be generated from the script.",
            entryPoint.contains("Generated by scripts/update_material_symbols.mjs"),
        )
        symbolDrawables.forEach { file ->
            val text = file.readText()
            assertTrue("${file.name} must use Android VectorDrawable XML.", text.contains("<vector"))
            assertTrue(
                "${file.name} must be generated from Material Symbols.",
                text.contains("Generated by scripts/update_material_symbols.mjs"),
            )
        }
    }
    ...
    fun `selected navigation icons use filled material symbol variants`() {
        ...
        val destinations = File(
            projectRoot,
            "app/src/main/java/com/driezy/medlog/ui/navigation/MedLogDestinations.kt",
        ).readText()
        val navigation = File(
            projectRoot,
            "app/src/main/java/com/driezy/medlog/ui/navigation/MedLogNavigationComponents.kt",
        ).readText()
        val expected = listOf(
            "HomeSelected",
            "HistorySelected",
        ...
        expected.forEach { property ->
            assertTrue(
                "Expected filled selected symbol property MedLogIcons.$property.",
                entryPoint.contains("val $property: Int"),
            )
            assertTrue(
                "Top-level destination should provide selected icon MedLogIcons.$property.",
                destinations.contains("MedLogIcons.$property"),
            )
        }
        assertTrue(
            "Navigation items should render selectedIcon when selected.",
            navigation.contains("if (selected) dest.selectedIcon else dest.icon"),
        )
        assertTrue(
            "Generator should fetch filled symbol variants.",
            File(projectRoot, "scripts/update_material_symbols.mjs").readText().contains("urlVariant: \"fill1\""),
        )
    }
    ...
    fun `large display icons use optical size material symbol variants`() {
        ...
        expected.forEach { (property, fileName) ->
            ...
            assertTrue(
                "Expected MedLogIcons.$property.",
                entryPoint.contains("val $property: Int = R.drawable.${fileName.removeSuffix(".xml")}"),
            )
        ...
        assertTrue(
            "Generator should fetch size-specific SVG endpoints.",
            scriptText.contains("""${'$'}{sizeDp}px.svg"""),
        )
    }
}
