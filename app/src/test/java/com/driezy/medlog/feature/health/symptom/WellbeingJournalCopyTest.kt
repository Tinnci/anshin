...
class WellbeingJournalCopyTest {
    ...
    fun `diary feature uses inclusive wellbeing language in Chinese resources`() {
        ...
        assertTrue(
            strings.contains(
                """<string name="symptom_dialog_supporting_text">不需要出现症状才记录；可以只记感受、身体变化、用药反应或备注。</string>""",
            ),
        )
    ...
    fun `diary feature uses inclusive wellbeing language in English resources`() {
        ...
        assertFalse(
            "User-facing copy should not call the feature Symptom Diary.",
            strings.contains("Symptom Diary"),
        )
        ...
        assertTrue(
            strings.contains(
                """<string name="symptom_dialog_supporting_text">You do not need symptoms to make an entry; record feelings, body changes, medication responses, or notes.</string>""",
            ),
        )
        assertTrue(strings.contains("""<string name="symptom_section_symptoms">Body signals (optional)</string>"""))
        assertTrue(
            strings.contains(
                """<string name="symptom_section_side_effects">Medication responses (optional)</string>""",
            ),
        )
    ...
    @Test
    fun `empty diary state uses bounded centered copy`() {
        val screen = source(
            "app/src/main/java/com/driezy/medlog/feature/health/symptom/SymptomDiaryScreen.kt",
        )

        assertTrue("Empty state should be a dedicated composable.", screen.contains("SymptomEmptyState("))
        assertTrue("Empty state copy stays centered.", screen.contains("textAlign = TextAlign.Center"))
        assertTrue("Empty state copy stays bounded.", screen.contains("widthIn(max = 300.dp)"))
    }
}
