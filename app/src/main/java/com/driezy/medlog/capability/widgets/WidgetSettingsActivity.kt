 ...
 *
 * SSOT：所有设置均通过 [SettingsWidgetViewModel] 写入同一 DataStore。
 */
@AndroidEntryPoint
class WidgetSettingsActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            val viewModel: SettingsWidgetViewModel = hiltViewModel()
            ...
            MedLogTheme(
                ...
                WidgetSettingsScreen(
                    ...
                    onShowActionsChange = {
                        viewModel.onAction(SettingsUiAction.SetWidgetShowActions(it))
                    },
                    onWidgetThemeModeChange = {
                        viewModel.onAction(SettingsUiAction.SetWidgetAppearance(themeMode = it))
                    },
                    onWidgetColorSourceChange = {
                        viewModel.onAction(SettingsUiAction.SetWidgetAppearance(colorSource = it))
                    },
                    onWidgetPaletteChange = {
                        viewModel.onAction(SettingsUiAction.SetWidgetAppearance(palette = it))
                    },
                    onWidgetDensityScaleChange = {
                        viewModel.onAction(SettingsUiAction.SetWidgetAppearance(densityScale = it))
                    },
                    onWidgetTextScaleChange = {
                        viewModel.onAction(SettingsUiAction.SetWidgetAppearance(textScale = it))
                    },
                    onCancel = {
                        val resultIntent = Intent().apply {
                            putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId)
