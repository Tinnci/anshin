...
class StreakWidget : GlanceAppWidget() {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        val rangeStart = today.minusDays(29).atStartOfDay(zone).toInstant().toEpochMilli()
        val rangeEnd = today.plusDays(1).atStartOfDay(zone).toInstant().toEpochMilli() - 1L
        val allLogs = ep.logRepository().getLogsForRangeOnce(rangeStart, rangeEnd)

        // 判断某天是否完成（所有计划药品均已标记 TAKEN）
        fun dayComplete(day: LocalDate): Boolean {
            if (total == 0) return false
            val dayStartMs = day.atStartOfDay(zone).toInstant().toEpochMilli()
            val dayEndMs = day.plusDays(1).atStartOfDay(zone).toInstant().toEpochMilli() - 1L
        ...
        val daysWithActivity = (0..29).mapNotNullTo(mutableSetOf()) { daysBack ->
            today.minusDays(daysBack.toLong()).takeIf(::dayComplete)
        }
        val streak = StreakCalculator.currentStreak(daysWithActivity, today)
        ...
        val dayData = (6 downTo 0).map { daysBack ->
            val localDate = today.minusDays(daysBack.toLong())
            ...
            Pair(dayComplete(localDate), label)
        }
