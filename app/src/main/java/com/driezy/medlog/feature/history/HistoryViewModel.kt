...
class HistoryViewModel @Inject constructor(
    ...
    private fun loadData() {
        viewModelScope.launch {
            ...
            val plannedItems = futurePlanCalculator.calculate(
                ...
                days = 91, // 含今天
                from = Instant.ofEpochMilli(startMs),
                zoneId = zone,
                includeArchived = true,
            )
            // 按日期分组计划条目
            val plannedByDay = plannedItems.groupBy { it.day }
    ...
    private fun buildAdherenceDay(
        ...
        dayPlanned: List<com.driezy.medlog.feature.medications.application.FuturePlanItem>,
        today: LocalDate,
        ...
        for (plan in dayPlanned) {
            ...
            val matchedLog = dayLogs.firstOrNull { log ->
                log.medicationId == plan.medication.id &&
                    ...
                    kotlin.math.abs(log.scheduledTimeMs - plan.scheduledAt.toEpochMilli()) < SLOT_WINDOW_MS
            }
            if (matchedLog != null) {
                ...
                mergedLogs.add(
                    MedicationLog(
                        ...
                        scheduledTimeMs = plan.scheduledAt.toEpochMilli(),
