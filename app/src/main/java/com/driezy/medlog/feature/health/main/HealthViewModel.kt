...
import com.driezy.medlog.feature.health.application.HealthInsight
import com.driezy.medlog.feature.health.application.HealthInsightGenerationUseCase
...
data class HealthTypeStat(
    ...
    val bloodPressureClass: BloodPressureClassification? = null,
)
...
data class HealthUiState(
    ...
    /** BMI（体重 + 身高可用时计算） */
    val bmi: Double? = null,
    val bmiClass: BmiClassification? = null,
    /** 用户身高（cm），0 = 未设置 */
...
sealed interface HealthUiAction {
    ...
    data class DraftTypeChanged(val type: HealthType) : HealthUiAction
    data class DraftValueChanged(val value: String) : HealthUiAction
    data class DraftSecondaryChanged(val value: String) : HealthUiAction
    data class DraftTimeChanged(val timestamp: Long) : HealthUiAction
    data class DraftNotesChanged(val value: String) : HealthUiAction
    data object VoiceInputStarted : HealthUiAction
    data object VoiceInputStopped : HealthUiAction
    data object RecordSaved : HealthUiAction
    data class DeleteRequested(val record: HealthRecord) : HealthUiAction
    data object DeleteCancelled : HealthUiAction
    data object DeleteConfirmed : HealthUiAction
}
...
class HealthViewModel @Inject constructor(
    ...
    fun onAction(action: HealthUiAction) {
        when (action) {
            ...
            is HealthUiAction.DraftTypeChanged -> onDraftTypeChange(action.type)
            is HealthUiAction.DraftValueChanged -> onDraftValueChange(action.value)
            is HealthUiAction.DraftSecondaryChanged -> onDraftSecondaryChange(action.value)
            is HealthUiAction.DraftTimeChanged -> onDraftTimeChange(action.timestamp)
            is HealthUiAction.DraftNotesChanged -> onDraftNotesChange(action.value)
            HealthUiAction.VoiceInputStarted -> startVoiceInput()
            HealthUiAction.VoiceInputStopped -> stopVoiceInput()
            HealthUiAction.RecordSaved -> saveRecord()
            is HealthUiAction.DeleteRequested -> requestDelete(action.record)
            HealthUiAction.DeleteCancelled -> cancelDelete()
            HealthUiAction.DeleteConfirmed -> confirmDelete()
        }
    }
    ...
    private fun collectStats() {
        viewModelScope.launch {
            ...
            combine(
                ...
                repository.getRecordsInRange(sevenDaysAgo, now),
                heightCmFlow,
                ...
                .collect { (stats, bmi, bmiClass) ->
                    _uiState.update { it.copy(stats = stats, bmi = bmi, bmiClass = bmiClass) }
                }
        }
        // 单独收集身高到 UI 状态
        viewModelScope.launch {
    ...
    private fun buildStats(latest: List<HealthRecord>, weekRecords: List<HealthRecord>): List<HealthTypeStat> {
        ...
        return HealthType.entries.mapNotNull { type ->
            ...
            // 血压分类
            val secondaryValue = rec.secondaryValue
            val bpClass = if (type == HealthType.BLOOD_PRESSURE && secondaryValue != null) {
                HealthType.classifyBloodPressure(rec.value, secondaryValue)
            } else {
                null
            }
    ...
    fun saveRecord() {
        ...
        val record = HealthRecord(
            ...
            confirmedAt = draft.confirmedAt ?: clock.millis(),
        )
        safeLaunch {
            stopVoiceInput()
