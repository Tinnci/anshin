...
@Composable
internal fun MedicationDateSection(uiState: AddMedicationUiState, onAction: (AddMedicationUiAction) -> Unit) {
    // ── 起止日期 ─────────────────────────────────────────
...
@Composable
internal fun MedicationStockSection(uiState: AddMedicationUiState, onAction: (AddMedicationUiAction) -> Unit) {
    // ── 库存管理 ─────────────────────────────────────────
    FormSection(title = stringResource(R.string.add_section_stock), icon = MedLogIcons.Inventory) {
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(MedLogSpacing.Small)) {
            OutlinedTextField(
                value = uiState.stock,
                onValueChange = { onAction(AddMedicationUiAction.StockChanged(it)) },
            ...
            OutlinedTextField(
                ...
                onValueChange = { onAction(AddMedicationUiAction.RefillThresholdChanged(it)) },
        ...
        Row(
            ...
            listOf(
                ...
                FilterChip(
                    ...
                    onClick = { onAction(AddMedicationUiAction.RefillReminderDaysChanged(days)) },
                    label = { Text(label) },
                )
            }
        }
...
@Composable
internal fun MedicationNotesSection(uiState: AddMedicationUiState, onAction: (AddMedicationUiAction) -> Unit) {
    // ── 备注 ─────────────────────────────────────────────
    FormSection(title = stringResource(R.string.add_section_notes), icon = MedLogIcons.Notes) {
        OutlinedTextField(
            value = uiState.notes,
            onValueChange = { onAction(AddMedicationUiAction.NotesChanged(it)) },
            ...
            trailingIcon = {
                VoiceInputTrailingIcon(
                    ...
                    onStartVoiceInput = { onAction(AddMedicationUiAction.StartVoiceInput) },
                    onStopVoiceInput = { onAction(AddMedicationUiAction.StopVoiceInput) },
                )
            },
            supportingText = {
