...
enum class AiFallbackReason {
    ...
    companion object {
        fun from(reason: HealthCloudImageAnalysisUnavailableReason): AiFallbackReason = when (reason) {
        ...
        fun from(reason: HealthCloudInsightUnavailableReason): AiFallbackReason = when (reason) {
...
data class AiExecutionStatus(
    ...
    companion object {
        ...
        fun unavailable(reason: AiFallbackReason): AiExecutionStatus = AiExecutionStatus(
            mode = AiExecutionMode.CLOUD_UNAVAILABLE_FALLBACK,
            reason = reason,
        )

        fun failed(reason: AiFallbackReason, error: Throwable? = null): AiExecutionStatus = AiExecutionStatus(
        ...
        ): AiExecutionStatus = when (error) {
            is AiProviderException -> providerError(error)
            else -> failed(fallbackReason, error)
        }

        fun providerError(error: AiProviderException): AiExecutionStatus = AiExecutionStatus(
            mode = AiExecutionMode.CLOUD_FAILED_FALLBACK,
